...
git config --global alias.cc '!f() { git checkout "$1" && git pull; }; f'
git config --global alias.dc '!f() { git branch --list "$1" && git branch -D "$1"; git checkout -b "$1"; }; f'
git config --global alias.cp "cherry-pick"
git config --global alias.co "commit"
git config --global alias.d "diff"
git config --global alias.ds "diff --staged"
git config --global alias.h '!git ls-files -v | grep "^[hS]"'
git config --global alias.l "log --oneline --graph -20 --pretty=format:'%C(Yellow)%h%C(reset) %s %C(auto)%d%C(reset) %C(dim yellow)%an%C(reset) %C(dim cyan)%ci%C(reset)'"
git config --global alias.ln "log --oneline --graph --no-merges -20 --pretty=format:'%C(Yellow)%h%C(reset) %s %C(auto)%d%C(reset) %C(dim yellow)%an%C(reset) %C(dim cyan)%ci%C(reset)'"
git config --global alias.ll "log --oneline --graph --all -20 --pretty=format:'%C(Yellow)%h%C(reset) %s %C(auto)%d%C(reset) %C(dim yellow)%an%C(reset) %C(dim cyan)%ci%C(reset)'"
git config --global alias.lln "log --oneline --graph --no-merges --all -20 --pretty=format:'%C(Yellow)%h%C(reset) %s %C(auto)%d%C(reset) %C(dim yellow)%an%C(reset) %C(dim cyan)%ci%C(reset)'"
...
## 커밋(Commit) 하나를 파일로 뽑기
1. GitHub에서 해당 Commit 페이지로 들어갑니다. (URL 끝이 .../commit/abcdef123... 형태)
2. 주소창의 URL 맨 끝에 .diff 또는 **.patch**를 타이핑하고 Enter를 누릅니다.
3. 예시: https://github.com/user/repo/commit/my_code_change.diff
4. 화면에 텍스트가 뜨면 Ctrl + S로 저장하거나 내용을 복사합니다.

## 커밋(Commit) 하나를 파일로 뽑은 파일 적용
```
patch -p1 < my_code_change.diff
```

## tag
1. 기본 태그 추가 (Lightweight Tag)
단순히 특정 지점에 포인터만 생성하는 방식입니다.

```
git tag <태그이름> <커밋해시>
예시: git tag v1.0.1 a1b2c3d
```

2. 주석이 포함된 태그 추가 (Annotated Tag)
메시지를 함께 저장하며, 생성자 정보와 날짜 등이 포함되어 권장되는 방식입니다.

```
git tag -a <태그이름> -m "태그 메시지" <커밋해시>
예시: git tag -a v1.1.0 -m "정식 배포 버전" a1b2c3d
```

3. 원격 저장소(GitHub 등)에 반영하기
로컬에서 생성한 태그는 별도로 push를 해줘야 서버에 반영됩니다.

```
git push origin <태그이름>
모든 태그를 한꺼번에 올리려면 다음 명령어를 사용하세요.
