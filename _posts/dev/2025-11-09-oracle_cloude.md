...
POSTGRES*PASSWORD=강력한*비밀번호*1로*변경
REDIS*PASSWORD=강력한*비밀번호*2로*변경
EOF
...
sudo mkdir -p /opt/moremong/{backend,frontend,frontend1,frontend2}
...
cd /opt/moremong/repo/moremong-restapi

# 프로덕션 환경 파일 생성

tee .env.production > /dev/null <<'EOF'
JASYPT*ENCRYPTOR_PASSWORD=여기에*마스터*비밀번호*입력
SPRING_PROFILES_ACTIVE=prod
SERVER_PORT=8090

# 데이터베이스

SPRING*DATASOURCE_URL=jdbc:postgresql://localhost:5432/moremong
SPRING_DATASOURCE_USERNAME=moremong
SPRING_DATASOURCE_PASSWORD=강력한*비밀번호*1로*변경

# Redis

SPRING*DATA_REDIS_HOST=localhost
SPRING_DATA_REDIS_PORT=6379
SPRING_DATA_REDIS_PASSWORD=강력한*비밀번호*2로*변경
...
application:
oauth2:
allowed-redirect-uris: - https://moremong.com/worklog/oauth/callback - https://moremong.com/worklog1/oauth/callback - https://moremong.com/worklog2/oauth/callback
...
command: - "postgres" - "-c" - "max_connections=200" - "-c" - "shared_buffers=256MB" - "-c" - "effective_cache_size=1GB" - "-c" - "maintenance_work_mem=64MB" - "-c" - "checkpoint_completion_target=0.9" - "-c" - "wal_buffers=16MB" - "-c" - "default_statistics_target=100"
EOF
