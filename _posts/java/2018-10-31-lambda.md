...
그 메소드 차이점을 찾으셨다면 당신은 칭찬합니다.  
람다식을 사용하면서 기존 Condition interface가 사라지고 Interface Predicate<T>로 대체했는데요.  
...
아래 링크(작명센스에 감탄할 수 밖에 없는) 참조하세요.  
[난파선 프로그래밍 블로그 - 표류중인 블로그](http://napasun-programming.tistory.com/25)
[난파선 프로그래밍 블로그 - 표류중인 블로그](http://napasun-programming.tistory.com/25 "Java - 자바에서 제공하는 함수형 인터페이스")
